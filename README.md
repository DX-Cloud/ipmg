...
| **IP历史记录恢复** | **支持恢复最近10条历史IP配置，自动去重，固定DHCP选项** |
| **设备搜索** | **支持按名称、IP模糊搜索设备，快速定位目标设备** |
...
2. **选择设备方式** — 可选择"显示全部设备"或"搜索设备"
3. **选择设备** — 从设备列表中选择，收藏的设备标有 `*`
4. **确认配置** — 显示网卡、设备IP、将要配置的网卡IP、子网掩码等信息
5. **自动备份** — 自动备份当前网卡的 IP 配置
6. **执行配置** — 通过 WMI 设置静态 IP
7. **Ping 验证** — 自动 Ping 设备检查连通性
8. **打开管理页面** — 可选择直接打开设备的 Web 管理页面
...
若选择搜索：
请输入搜索关键词（支持设备名称、IP模糊匹配）:
关键词: 交换

搜索结果: 2 条匹配
-------------------------------------------------------
   1. [*] 核心交换机 | 设备: 10.251.251.251 | -> 网卡IP: 10.251.251.254
   2. [ ] 接入交换机 | 设备: 192.168.1.1 | -> 网卡IP: 192.168.1.254
   0. <-- 返回上一页
-------------------------------------------------------
```

**搜索特性：**
- 支持设备名称模糊匹配（如"交换"可匹配"核心交换机"、"接入交换机"）
- 支持设备IP模糊匹配（如"251"可匹配"10.251.251.251"、"192.168.251.1"）
- 不区分大小写
- 搜索结果为空时可重新搜索或返回

### 恢复IP流程

1. **选择网卡** — 显示所有网卡，选择要恢复IP的网卡
2. **选择恢复方式** — 显示历史记录列表供选择：
   - 第一项固定为 "恢复为 DHCP（自动获取）"
   - 后续显示最近10条静态IP历史记录（IP/掩码/网关/时间戳）
3. **确认恢复** — 确认后执行恢复
...
-------------------------------------------------------
   1. 恢复为 DHCP（自动获取）
   2. 192.168.1.100 / 255.255.255.0  网关: 192.168.1.1  [2024-01-15 14:30]
   3. 10.251.251.254 / 255.255.255.0  网关: 无  [2024-01-15 10:20]
   4. 192.168.0.50 / 255.255.255.0  网关: 192.168.0.1  [2024-01-14 16:45]
   0. <-- 返回上一页
-------------------------------------------------------
```

**历史记录特性：**
- 每次配置IP前，自动将当前静态IP保存到历史记录
- 最多保存10条历史记录，超出自动删除最旧的
- 相同IP配置（IP+掩码+网关）自动合并，更新时间戳
- DHCP配置不记录到历史（有专门的DHCP选项）
- 历史记录按网卡MAC分别存储
